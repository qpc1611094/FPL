...
from semseg.utils.loss_helper_topk import get_criterion
...
def main():
    ...
    print ('*********************************************************')
    print (len(trainloader_sup), len(trainloader_unsup), len(valloader))
        ...
        if cfg_trainer["eval_on"]:
            ...
            if rank == 0:
                if prec > best_prec:
                    ...
                    torch.save(state, osp.join(cfg['saver']['snapshot_dir'], str(prec)+'_best_'+str(seed)+'.pth'))
...
#             print ('sup:',preds_teacher_sup.shape)
#             print ('unsup:',preds_teacher_unsup.shape)
            #compute consistency loss
            logits_teacher_sup = preds_teacher_sup.detach()            
...
#             logits_teacher_sup[conf_teacher_sup_map < threshold] = 255~~~~~~~~~~~~
            ...
            logits_teacher_unsup = preds_teacher_unsup.detach()
#             if not cutmix and not acm:
...
#             logits_teacher_unsup[conf_teacher_unsup_map < threshold] = 255~~~~~~~~~~~~~~~
        ...
        if isinstance(criterion_cons, torch.nn.CrossEntropyLoss):
            loss_consistency1 = criterion_cons(preds_student_sup[0],logits_teacher_sup.max(1))/world_size
            loss_consistency2 = criterion_cons(preds_student_unsup[0],logits_teacher_unsup.max(1))/world_size
        elif sample:
            loss_consistency1, scalar_sup = criterion_cons(preds_student_sup[0],conf_sup, logits_teacher_sup, class_criterion[0])
            loss_consistency2, scalar_unsup = criterion_cons(preds_student_unsup[0],conf_unsup, logits_teacher_unsup, class_criterion[0])
            
            loss_consistency1, scalar_sup = loss_consistency1/world_size, scalar_sup/world_size
            loss_consistency2, scalar_unsup = loss_consistency2/world_size, scalar_unsup/world_size
        else:
            loss_consistency1, scalar_sup = criterion_cons(preds_student_sup[0],conf_sup, logits_teacher_sup)
            loss_consistency2, scalar_unsup = criterion_cons(preds_student_unsup[0],conf_unsup, logits_teacher_unsup)
        ...
        scalar_unsup = torch.from_numpy(np.array(scalar_unsup)).cuda()
        loss_sup = torch.from_numpy(np.array(loss_sup_student.item())).cuda()

        dist.all_reduce(reduced_intersection)
        dist.all_reduce(reduced_union)
        dist.all_reduce(reduced_target)
        dist.all_reduce(scalar_unsup)
        dist.all_reduce(loss_sup)

        intersection_meter.update(reduced_intersection.cpu().numpy())
        union_meter.update(reduced_union.cpu().numpy())
        target_meter.update(reduced_target.cpu().numpy())
        scalar_meter.update(scalar_unsup)
        loss_sup_meter.update(loss_sup)
        ...
        if i_iter % 50 == 0 and rank==0:
            ...
            logger.info('iter = {} of {} completed, LR = {} loss = {}, mIoU = {}, scalar={}, loss_sup={}'
                        .format(i_iter, cfg['trainer']['epochs']*len(data_loader_unsup), lr, losses.avg, mIoU, scalar_meter.avg, loss_sup_meter.avg))
