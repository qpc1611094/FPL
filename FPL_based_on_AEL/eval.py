...
def main():
    ...
    if "cityscapes" in cfg['dataset']['type']:
        for line in open(f_data_list, 'r'):
            ...
            arr = [data_root + item for item in arr]
    #         print ('->>>>>>>>>', arr)
            data_list.append(arr)
    else:
        for line in open(f_data_list, 'r'):
            arr = line.strip()
            arr = [os.path.join(data_root,'image',arr+'.jpg'), os.path.join(data_root,'label',arr+'.png')]
#             arr = [data_root + item for item in arr]
#             print ('->>>>>>>>>', arr)
            data_list.append(arr)
...
def cal_acc(data_list, pred_folder, classes):
    ...
    for i, (image_path, target_path) in enumerate(data_list):
        ...
        accuracy = sum(intersection_meter.val) / (sum(target_meter.val) + 1e-10) #target = TP+FN
        
#         FN = sum(target_meter.val) - sum(intersection_meter.val)
#         FP = sum(union_meter.val) - sum(intersection_meter.val) - FN
#         TP = sum(intersection_meter.val)
...
#               .format(i + 1, len(data_list), image_name+'.png', accuracy))
    ...
    TP = intersection_meter.sum
    FN = target_meter.sum - TP
    FP = union_meter.sum - TP - FN
    precision = TP / (TP + FP)
    recall = TP / (TP + FN)
    ...
    print('Eval result: mIoU/mAcc/mprecision/mrecall/allAcc {:.4f}/{:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(mIoU, mAcc, mprecision, mrecall, allAcc))
    for i in range(classes):
        print('Class_{} result: iou/accuracy/precision/recall {:.4f}/{:.4f}/{:.4f}/{:.4f}'.format(i, iou_class[i], accuracy_class[i],precision[i],recall[i]))
