...
class voc_dset(BaseDataset):
    def __init__(self, data_root, data_list, trs_form, seed=0, n_sup=10582, split='val',unsup=False, fm=False, acp=False, paste_trs=None, prob=0.5, acm=False):
        ...
        random.seed(seed)
#         if split=='train':
#             self.list_sample_new = random.sample(self.list_sample, len(self.list_sample))
#             if unsup:
#                 # transform to tuple
# #                 for i in range(len(self.list_sample)):
# # #                     print ('->>>>>>>>>>>>', self.list_sample[i])
# #                     self.list_sample[i]=self.list_sample[i][0]
...
#         else:
        self.list_sample_new = [item[0] for item in self.list_sample]
...
#         label_path = os.path.join(self.data_root, self.list_sample_new[index][1])
        image_path = os.path.join(self.data_root, 'image', self.list_sample_new[index]+'.jpg')
        label_path = os.path.join(self.data_root, 'label', self.list_sample_new[index]+'.png')
#         print ('->>>>>>>>>>>>', len(self.list_sample_new))
#         print ('->>>>>>>>>>>>', index)
#         print ('->>>>>>>>>>>>', self.list_sample_new[index])
#         print ('->>>>>>>>>>>>', self.data_root)
#         image_path = self.data_root + self.list_sample_new[index][0]
#         label_path = self.data_root + self.list_sample_new[index][1]
#         print ('->>>>>>>>>>>>', image_path)
#         print ('->>>>>>>>>>>>', label_path)
        ...
        if self.acp:
            if random.random()>self.prob:
                ...
                paste_img_path = os.path.join(self.data_root, 'image', self.list_sample_new[index]+'.jpg')
#                 paste_img_path = self.data_root + self.list_sample_new[paste_idx][0]
                paste_img = self.img_loader(paste_img_path, 'RGB')
                paste_label_path = os.path.join(self.data_root, 'label', self.list_sample_new[index]+'.png')
#                 paste_label_path = self.data_root + self.list_sample_new[paste_idx][1]
...
def build_voc_semi_loader_cp(split, all_cfg,  seed=0):
    ...
    if acp:
        ...
        paste_trs = None
    dset = voc_dset(cfg['data_root'], cfg['data_list_sup'], trs_form, seed, n_sup, split, acp=acp, paste_trs=paste_trs, prob=prob)
    ...
    dset_unsup = voc_dset(cfg['data_root'], cfg['data_list_uns'], trs_form_unsup, seed, n_sup, split, unsup=True, fm=fm, acm=acm)
    if split == 'train':
