...
def context_prob_weight(multi_hot_label_expand, size=9):
#     multi_hot_label_expand b,c,w,h,MC_num
    center_density = 1e-12 + multi_hot_label_expand / multi_hot_label_expand.sum(dim=1, keepdim=True)
    
    b,c,w,h,MC_num = multi_hot_label_expand.shape
    kernel = torch.ones(c, 1, size, size, 1).cuda()
    kernel[:,:,size//2, size//2,:] = 0
    zeros = torch.zeros(b,w,h,1).cuda()
    label_box = F.conv3d(multi_hot_label_expand, kernel, stride=1, padding=(size//2, size//2, 0),groups=c) 
    
    box_density = 1e-12 + label_box / label_box.sum(dim=1, keepdim=True) #b,c,w,h,MC_num
    density_kl = F.kl_div(center_density.log(), box_density, reduction='none').sum(1) #b,w,h,MC_num
    label_cover_rate_index = density_kl.argmin(-1) #b,w,h
    
    label_cover_weight = to_one_hot(label_cover_rate_index, nClasses=MC_num).permute(0,2,3,1)
    
    return label_cover_weight

def semi_ce_loss_FPL_expand_MC_beyas_dilated(pred, inputs_2_o, plabel, temp_k=3, threshold=[0.85,0.9,0.95,0.99], weight='concave', temp=1.0):
    
    inputs_2 = F.interpolate(inputs_2_o, scale_factor=1.0, mode='bilinear', align_corners=True)
    soft_plus = torch.nn.Softplus()
    prob_topk, pse_topk = torch.topk(inputs_2.float().softmax(dim=1).detach().pow(temp), k=temp_k + 1, dim=1) #b,c,w//4,h//4
    class_num = pred.shape[1]

    k_max = pse_topk.shape[1]
    one_hot_dict = {}
    ...
    MC_threshold = torch.from_numpy(np.array(threshold)).cuda()
    MC_num = len(MC_threshold)
    MC_threshold_expand = MC_threshold[None, None, None, :]
    flag_mask = torch.zeros([inputs_2.shape[0], inputs_2.shape[2], inputs_2.shape[3], MC_num]).cuda() - 1
    weight_mask = torch.zeros_like(flag_mask) + 1
    multi_hot_label = torch.zeros([inputs_2.shape[0], inputs_2.shape[1], inputs_2.shape[2], inputs_2.shape[3], MC_num]).cuda()  
    
    cumulative_prob_expand = cumulative_prob[:, :, :, :, None].expand(inputs_2.shape[0], k_max, inputs_2.shape[2], inputs_2.shape[3],
                                                                          MC_num).contiguous()  # b,c,w,h,MC_num
    for i in range(1, cumulative_prob.shape[1]):
        k = i - 1  # i-1 form 0
        ...
        if i == cumulative_prob.shape[1] - 1:
            ...
            mask_k = ((cumulative_prob_expand[:, i, :, :, :] - MC_threshold_expand)>=0) * (flag_mask == -1) # b,w,h,MC_num
        flag_mask[mask_k] = 1


        if weight == 'linear':
            weight_mask[mask_k] = ((cumulative_prob_expand[:, i, :, :, :] / (i + 1) - prob_topk[:, i, :, :, None]) / (
                        cumulative_prob_expand[:, i, :, :, :] / ((i + 1))))[mask_k]
        elif weight == 'convex':
            weight_mask[mask_k] = ((cumulative_prob_expand[:, i, :, :, :] / (i + 1) - prob_topk[:, i, :, :, None]) / (
                        prob_topk[:, i, :, :, None] + cumulative_prob_expand[:, i, :, :, :] / (i + 1)))[mask_k]
        elif weight == 'concave':
            weight_mask[mask_k] = (torch.log(
                1 + cumulative_prob_expand[:, i, :, :, :] / (i + 1) * 50 - prob_topk[:, i, :, :, None] * 50) / torch.log(
                1 + 50 * cumulative_prob_expand[:, i, :, :, :] / ((i + 1))))[mask_k]
        
        
        
        mask_k = mask_k[:, None, :, :, :].expand(inputs_2.shape[0], class_num, inputs_2.shape[2], inputs_2.shape[3], MC_num).contiguous()

        pse_mask_pos = 0
        for j in range(k + 1):
            pse_mask_pos += one_hot_dict[j]
        pse_mask_pos = pse_mask_pos[:,:,:,:,None].expand(inputs_2.shape[0], class_num, inputs_2.shape[2], inputs_2.shape[3], MC_num) #b,c,w,h,MC_num
        multi_hot_label[mask_k] = pse_mask_pos[mask_k] #b,c,w,h,MC_num
    
    if MC_num > 1:
        context_weight = context_prob_weight(multi_hot_label.clone(), size=5) #size=7 b,w,h,MC
    else:
        context_weight = torch.ones_like(multi_hot_label[:,0,:,:,:])
    
    multi_hot_label = (multi_hot_label * context_weight[:,None,:,:,:]).sum(-1) #b,c,w,h
    multi_hot_label_state = multi_hot_label.sum(1)
    multi_hot_label = F.interpolate(multi_hot_label, scale_factor=1, mode='nearest')
    weight_mask = (weight_mask * context_weight).sum(-1)
    weight_mask = F.interpolate(weight_mask[:,None,:,:], scale_factor=1, mode='bilinear', align_corners=True)[:,0]
    
    sum_pred_pos = (-(pred * multi_hot_label) - (1-multi_hot_label) * 1e12)
    sum_pred_neg = ((pred * (1-multi_hot_label)) - multi_hot_label * 1e12)
    
    
    loss_topk = soft_plus(torch.logsumexp(sum_pred_pos, dim=1) + torch.logsumexp(sum_pred_neg, dim=1)) #b,w,h,MC_num
    
    
    

    
    useful_mask = (plabel==255)
    
    loss_FPL_for_scalar = loss_topk * useful_mask
    loss_base = criterion_ce_mask(pred.detach(), inputs_2_o.argmax(1))
    loss_base = loss_base * useful_mask
    scalar_total = loss_base.mean().item() / loss_FPL_for_scalar.mean().item()
    
    loss_FPL_topk = loss_topk * useful_mask * weight_mask
    
    Tu_mask = (context_weight * MC_threshold).sum(-1)
    
    Tu_value_mean = Tu_mask[useful_mask].mean()
    K_value_mean = multi_hot_label_state[useful_mask].mean()
    
    label1_rate = (multi_hot_label_state[useful_mask]==1).float().mean()

    
    return loss_FPL_topk, Tu_value_mean, K_value_mean, scalar_total, label1_rate

criterion_ce_mask = nn.CrossEntropyLoss(ignore_index=255, reduction='none')


def entropy_mini(prob):
    e = - (prob * prob.log()).mean()
    return e


def compute_loss_total(pred, mix_label, logit, pseudo_label, neg_label, pesudo_n_loss):
    loss_fpl_topk, Tu_value_mean, K_value_mean, scalar_total, label1_rate = semi_ce_loss_FPL_expand_MC_beyas_dilated(pred, logit, mix_label, temp_k=4, threshold=[0.84,0.89,0.94,0.99], weight='concave', temp=1.0) #b,w,h
    loss_fpl = (loss_fpl_topk * (mix_label==255).float()).mean()
    return loss_fpl, Tu_value_mean, K_value_mean, scalar_total, label1_rate
    ...
    for rebuild_id in range(255):
        ...
        for i, (tgt_input, y, names, tgt_trans_param, tgt_img_full, mix_label) in enumerate(tgt_train_loader):
            ...
            with torch.no_grad():   
                ...
                tgt_pred_his = full2weak(tgt_pred_his_full, tgt_trans_param)
                tgt_prob_his = F.softmax(tgt_pred_his, dim=1)
            ...
            original_weight = (-(1-uc_map_eln)).exp()
            
            
            if iteration<=begin_fpl:
                ...
                tgt_pred = tgt_pred
                tgt_pred_his = tgt_pred_his
                loss_fpl_topk, Tu_value_mean, K_value_mean, scalar_total, label1_rate = compute_loss_total(tgt_pred, mix_label, tgt_pred_his, psd_label, t_neg_label, pesudo_n_loss.detach())
                
                st_loss = criterion(tgt_pred, psd_label.long())
                pesudo_p_loss = (st_loss * (-(1-uc_map_eln)).exp() ).mean()
                pesudo_n_loss = binary_ce(tgt_pred.view(m_batchsize*C, 1, height, width), t_neg_label) * 0.5
                st_loss = pesudo_p_loss + loss_fpl_topk * 0.01
            ...
            meters.update(loss_total=st_loss.item())
            if not iteration<=begin_fpl:
                meters.update(Tu_value_mean=Tu_value_mean.item())
                meters.update(K_value_mean=K_value_mean.item())
                meters.update(scalar = scalar_total)
                meters.update(label1_rate = label1_rate.item())
...
def main():
    ...
    logger = setup_logger("DTST", output_dir, args.local_rank, filename="log_fpl.txt")
