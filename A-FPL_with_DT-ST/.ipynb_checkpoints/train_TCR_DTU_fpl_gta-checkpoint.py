...
def context_prob_weight(multi_hot_label_expand, size=9):
    center_density = 1e-6 + multi_hot_label_expand / multi_hot_label_expand.sum(dim=1, keepdim=True)
    ...
    box_density = 1e-6 + label_box / label_box.sum(dim=1, keepdim=True) #b,c,w,h,MC_num
    ...
    return label_cover_weight.detach()

def semi_ce_loss_FPL_expand_MC_beyas_dilated(pred, inputs_2_o, plabel, temp_k=3, threshold=[0.85,0.9,0.95,0.99], weight='concave'):
    
    inputs_2 = F.interpolate(inputs_2_o, scale_factor=1.0, mode='bilinear', align_corners=True)
    soft_plus = torch.nn.Softplus()
    prob_topk, pse_topk = torch.topk(inputs_2.float().softmax(dim=1).detach(), k=temp_k + 1, dim=1) #b,c,w//4,h//4
    ...
    flag_mask = torch.zeros([inputs_2.shape[0], inputs_2.shape[2], inputs_2.shape[3], MC_num]).cuda() - 1 #b,c,w,h,MC
    weight_mask = torch.zeros_like(flag_mask) + 1
    multi_hot_label = torch.zeros([inputs_2.shape[0], inputs_2.shape[1], inputs_2.shape[2], inputs_2.shape[3], MC_num]).cuda() #b,c,w,h,MC
    ...
    sum_pred_pos = (-(pred * multi_hot_label) - (1-multi_hot_label) * 1e7)
    sum_pred_neg = ((pred * (1-multi_hot_label)) - multi_hot_label * 1e7)
...
def compute_loss_total(pred, mix_label, logit, pseudo_label, neg_label):
    loss_fpl_topk = semi_ce_loss_FPL_expand_MC_beyas_dilated(pred, logit, mix_label, temp_k=4, threshold=[0.84,0.89,0.94,0.99], weight='concave') #b,w,h
    loss_top1 = criterion_ce_mask(pred, mix_label.long())
    st_loss = (loss_fpl_topk * (mix_label==255) + loss_top1 * (mix_label!=255)).mean()
    return st_loss
...
def train(cfg, local_rank, distributed):
    ...
    for rebuild_id in range(255):
        ...
        for i, (tgt_input, y, names, tgt_trans_param, tgt_img_full, mix_label) in enumerate(tgt_train_loader):
            ...
            if iteration<=4000: #first 4000 iteration used the base loss
                ...
                st_loss = compute_loss_total(tgt_pred, mix_label, tgt_pred_his, psd_label, t_neg_label)
                st_loss = st_loss
