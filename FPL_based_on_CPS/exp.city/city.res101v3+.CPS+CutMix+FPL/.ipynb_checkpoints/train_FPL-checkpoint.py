...
print (config.threshold, config.weight)
...
with Engine(custom_parser=parser) as engine:
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            loss_topk_12, weight_mask_12 = topk_ce_FPL(logits_cons_stu_1, logits_cons_tea_2, threshold = config.threshold, weight=config.weight)
            loss_topk_21, weight_mask_21 = topk_ce_FPL(logits_cons_stu_2, logits_cons_tea_1, threshold = config.threshold, weight=config.weight)
            cps_loss_base = criterion_cps(logits_cons_stu_1, ps_label_2) + criterion_cps(logits_cons_stu_2, ps_label_1)
            cps_scalar = cps_loss_base.item()/(loss_topk_12.mean()+loss_topk_21.mean()).item() # converge to >0.93 is usually good

            cps_loss = (loss_topk_12*weight_mask_12).mean()+(loss_topk_21*weight_mask_21).mean()
            cps_weight_balance = cps_loss_base.item()/cps_loss.item() # keep loss balance
            cps_loss = cps_loss * cps_weight_balance
            
            
            dist.all_reduce(cps_loss, dist.ReduceOp.SUM)
            cps_loss = cps_loss / engine.world_size
            cps_loss = cps_loss * config.cps_weight
#             cps_loss = cps_loss * cps_scalar
        ...
        if engine.distributed and (engine.local_rank == 0):
            ...
            logger.add_scalar('cps_scalar', sum_scalar / len(pbar), epoch)
        ...
        if azure and engine.local_rank == 0:
            ...
            run.log(name='Scalar CPS', value=sum_scalar / len(pbar))

        if (epoch > 0) and (epoch % config.snapshot_iter == 0) or (epoch == config.nepochs - 1):
            if engine.distributed and (engine.local_rank == 0):
